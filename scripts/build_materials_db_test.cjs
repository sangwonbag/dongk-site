...
function applyRules(category, brand, line, fileName, nameOnly, id, code, brandFolder) {
    ...
    // -- Exceptions / Spec Logic -- 
    if (category === '장판' && brand === 'LX') {
        const thicknessSource = brandFolder || line;
        const tMatch = thicknessSource.match(/\d\.\dT/i);
        if (tMatch) {
    ...
    files.forEach(file => {
        ...
        if (file.isDirectory()) {
            ...
            let cleanName = nameOnly.replace(/^\d+\.\s*/, '').replace(/^내지_데코_/, '')
                              ...
                              .replace(/_\d+$/, '') // strip _0, _1, _2
                              .replace(/_$/, ''); // strip trailing underscore
                              
            let finalCode = cleanName.split(' ')[0]; // usually the first word is the code
            ...
            // De-dupe check 
            if (category === 장판) console.log(finalCode, rules.thickness); const existingIdx = items.findIndex(i => i.id === id);
